jQuery( document ).ready( function( $ ) {
	...
	ALPHA_Admin_Shortcode_Generator = {

		// Cache jQuery objects
		$cached: {
			download_dropdown: 	$( '#alpha-select-download-dropdown' ),
			style_dropdown: 	$( '#alpha-select-style-dropdown' ),
			button_dropdown: 	$( '#alpha-select-button-dropdown' ),
			button_container: 	$( '#alpha-button-dropdown-container' ),
			count_button: 		$( '#alpha-download-count' ),
			file_size_button: 	$( '#alpha-file-size' )
		},
		...
		insert: function() {
			...
			$( '#alpha-insert' ).on( 'click', function( e ) {
				...
				// Add text
				if ( $( '#alpha-custom-text' ).val().length > 0 ) {
					attrs += ' text="' + $( '#alpha-custom-text' ).val() + '"';
				}
	...
	ALPHA_Admin_Shortcode_Generator.init();
