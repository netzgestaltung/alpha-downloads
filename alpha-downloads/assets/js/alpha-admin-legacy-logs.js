jQuery( document ).ready( function( $ ) {
	 ...
	 * statistics screen.
	 */
	var ALPHA_Legacy_Logs = {

		// Store options from WP serialized array
		options: {},

		migrateContainer : $( '#alpha_migrate_message' ),
		migrateButton: $( '#alpha_migrate_button' ),
		migrateSpinner: $('#alpha_migrate_message .spinner'),
		migrateCount: $('#alpha_migrate_count'),
		...
		toggle: function() {
			...
			this.migrateButton.on( 'click', function() {
				...
				if ( false === self.migrating || 'undefined' === typeof self.migrating ) {
					$( document ).trigger( 'alpha_migrate_start' );
				}
				else {
					$( document ).trigger( 'alpha_migrate_stop' );
				}
		...
		start: function() {
			...
			$( document ).on( 'alpha_migrate_start', function() {
				...
				$.ajax( {
					...
					success: function( response ) {
						...
						if ( 'success' === response.status ) {
							$( document ).trigger( 'alpha_migrate_success', response.content );
						}
						// Error, trigger error
						else {
							$( document ).trigger( 'alpha_migrate_error' );
						}
					}
				} );
		...
		stop: function() {
			...
			$( document ).on( 'alpha_migrate_stop', function() {
				// Change button and set migrating flag
		...
		success: function() {
			...
			$( document ).on( 'alpha_migrate_success', function( e, current_count ) {
		...
		error: function() {
			...
			$( document ).on( 'alpha_migrate_error', function() {
				$( document ).trigger( 'alpha_migrate_stop' );
	...
	if ( 'undefined' !== typeof alpha_admin_logs_migrate ) {
		ALPHA_Legacy_Logs.init( alpha_admin_logs_migrate );
	}
