...
class ALPHA_Statistics {
	...
	public function count_downloads( $args = array() ) {
		...
		// Create cache key
		$key = 'alpha_count_days' . absint( $days ) . 'id' . absint( $download_id );
	...
	function get_popular_downloads( $args = array() ) {
		...
		// Create cache key
		$key = 'alpha_popular_days' . absint( $days ) . 'limit' . absint( $limit );
...
$GLOBALS['alpha_statistics'] = new ALPHA_Statistics();
