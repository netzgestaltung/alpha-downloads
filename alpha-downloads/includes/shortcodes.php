 ...
 * @return string
 */
function alpha_shortcode_ddownload( $atts ) {
	/**
	...
	extract( shortcode_atts(
		array(
			...
			'style'	=> $alpha_options['default_style'],
			'button'=> '',
			'color'	=> '', // Deprecated
			'class'	=> ''
		), $atts, 'ddownload' )
	...
	if ( $id == '' || !alpha_download_valid( $id ) ) {
		
		return __( 'Invalid download ID.', 'alpha-downloads' );
	}

	// Check style against registered styles
	$registered_styles = alpha_get_shortcode_styles();
	...
	if ( array_key_exists( $style, $registered_styles ) ) {
		...
		return __( 'Invalid style attribute.', 'alpha-downloads' );
	}
	...
	if ( $style == 'button' ) {
		...
		$button = ( empty( $button ) ) ? $alpha_options['default_button'] : $button;
		$registered_buttons = alpha_get_shortcode_buttons();
		...
		if ( array_key_exists( $button, $registered_buttons ) ) {
			...
			return __( 'Invalid button attribute.', 'alpha-downloads' );
		}
	}
	...
	$classes .= ' ext-' . alpha_get_file_ext( get_post_meta( $id, '_alpha_file_url', true ) ); // File extension
	...
	$output = alpha_search_replace_wildcards( $style_format, $id );

	return apply_filters( 'alpha_shortcode_ddownload', $output, $id, $atts, $classes );
}
add_shortcode( 'ddownload', 'alpha_shortcode_ddownload' );
 ...
 * @since   1.3
 */
function alpha_shortcode_ddownload_list( $atts ) {
	...
	extract( shortcode_atts(
		array(
			...
			'exclude_tags'			=> '',
			'relation'				=> 'AND',
			'style'					=> $alpha_options['default_list'],
			'cache'					=> true,
		), $atts, 'ddownload_list' ) 
	);
	...
	if ( !in_array( strtolower( $orderby ), array( 'title', 'date', 'count', 'filesize', 'random' ) ) ) {
		...
		return __( 'Invalid orderby attribute.', 'alpha-downloads' );
	}
	else {
		switch ( $orderby ) {
			case 'title':
			...
			case 'count':
				$query_args['meta_key'] = '_alpha_file_count';
				$query_args['orderby'] = 'meta_value_num';
				break;

			case 'filesize':
				$query_args['meta_key'] = '_alpha_file_size';
				$query_args['orderby'] = 'meta_value_num';
				break;
	...
	if ( !in_array( strtoupper( $order ), array( 'ASC', 'DESC' ) ) ) {
		...
		return __( 'Invalid order attribute.', 'alpha-downloads' );
	}
	else {
	...
	if ( !in_array( strtoupper( $relation ), array( 'AND', 'OR' ) ) ) {
		...
		return __( 'Invalid relation attribute.', 'alpha-downloads' );
	}
	else {
	...
	// Check style against registered styles
	$registered_styles = alpha_get_shortcode_lists();
	...
	$key = substr( 'alpha_shortcode_list_' . $key, 0, 45 );

	$alpha_cache = new ALPHA_Cache( $key );

	if ( true == $cache && false !== ( $cached_data = $alpha_cache->get() ) ) {
		...
		if ( $downloads_list->have_posts() ) {
			...
			while ( $downloads_list->have_posts() ) {
				...
				$classes .= ' ext-' . alpha_get_file_ext( get_post_meta( get_the_ID(), '_alpha_file_url', true ) ); // File extension

				$new_style_format = str_replace( '%class%', $classes, $style_format );

				echo '<li>' . alpha_search_replace_wildcards( $new_style_format, get_the_ID() ) . '</li>';
			...
			if ( true == $cache ) {
				...
				$alpha_cache->set( $output );
			}
		}
		else {
			return '<p>' . __( 'No downloads found.', 'alpha-downloads' ) . '</p>';
		}

	}
	
	return apply_filters( 'alpha_shortcode_ddownload_list', $output );
}
add_shortcode( 'ddownload_list', 'alpha_shortcode_ddownload_list' );
 ...
 * @since   1.0
 */
function alpha_shortcode_ddownload_filesize( $atts ) {
	...
	if ( $id !== false && !alpha_download_valid( $id ) ) {
		
		return __( 'Invalid download ID.', 'alpha-downloads' );
	}

	// First check for cached data
	$key = 'alpha_shortcode_filesize_id' . absint( $id );
	...
	if ( true == $cache && false !== ( $cached_data = $alpha_cache->get() ) ) {
		...
		$filesize = alpha_get_filesize( $id );
	}

	// Save to cache
	if ( true == $cache ) {
		
		$alpha_cache->set( $filesize );
	}
	...
	return apply_filters( 'alpha_shortcode_ddownload_filesize', $filesize );
}
add_shortcode( 'ddownload_filesize', 'alpha_shortcode_ddownload_filesize' );
add_shortcode( 'ddownload_total_filesize', 'alpha_shortcode_ddownload_filesize' ); // Deprecated
add_shortcode( 'ddownload_size', 'alpha_shortcode_ddownload_filesize' ); // Deprecated
 ...
 * @since   1.0
 */
function alpha_shortcode_ddownload_count( $atts ) {
	...
	// No need to check cache, caching handled by alpha_statistics

	// Get count
	$count = $alpha_statistics->count_downloads( array( 
		'download_id'	=> $id, 
		'days'			=> $days,
		'cache' 		=> $cache 
	) );
	...
	return apply_filters( 'alpha_shortcode_ddownload_count', $count );
}
add_shortcode( 'ddownload_count', 'alpha_shortcode_ddownload_count' );
add_shortcode( 'ddownload_total_count', 'alpha_shortcode_ddownload_count' ); // Deprecated
 ...
 * @since   1.3
 */
function alpha_shortcode_ddownload_files( $atts ) {
	global $alpha_options;
	...
	if ( true == $cache && false !== ( $cached_data = $alpha_cache->get() ) ) {
		...
		// No cached data, retrieve file count
		$total_files = wp_count_posts( 'alpha_download' );
	...
	// Apply filters and return
	return apply_filters( 'alpha_shortcode_ddownload_files', $total_files );
}
add_shortcode( 'ddownload_files', 'alpha_shortcode_ddownload_files' );
add_shortcode( 'ddownload_total_files', 'alpha_shortcode_ddownload_files' ); // Deprecated
