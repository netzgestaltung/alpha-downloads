...
class ALPHA_Logging {
	...
	public function insert_log( $log ) {
		...
		if ( $wpdb->query( $sql ) && 'success' === $log['status'] ) {
			...
			$count = get_post_meta( $log['post_id'], '_alpha_file_count', true );
			update_post_meta( $log['post_id'], '_alpha_file_count', ++$count );
		}
	...
	public function role_check( $log ) {
		...
		if ( current_user_can( 'administrator' ) && !$alpha_options['log_admin_downloads'] ) {
			return false;
		}
	...
	public function grace_period( $log ) {
		...
		if ( $alpha_options['grace_period'] == 1 ) {
			...
			$log['post_id'],
			$log['date'],
			$alpha_options['grace_period_duration'],
			$this->prepare_ip_address( $log['ip_address'] ) );
...
$GLOBALS['alpha_logging'] = new ALPHA_Logging();
