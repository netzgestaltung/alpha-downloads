...
// Include Alpha Downloads
include_once( 'alpha-downloads.php' ); 

// Globals
global $wpdb, $alpha_options, $alpha_statistics;

// Clear transients
alpha_delete_all_transients();

// Check for complete uninstall
if ( $alpha_options['uninstall'] ) {
	...
	*/
	if ( function_exists( 'alpha_download_taxonomies' ) ) {
		alpha_download_taxonomies();
	}
	...
	$alpha_statistics->delete_table();

	// Delete options
	delete_option( 'alpha-downloads' );
	delete_option( 'alpha-downloads-version' );
	delete_option( 'alpha-downloads-prior-version' );
	delete_option( 'alpha-downloads-notices' );
