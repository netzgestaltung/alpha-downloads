...
abstract class Alpha_Downloads_Addon {
	/**
	 ...
	 * @param string $slug
	 *
	 * @return Alpha_Downloads_Customizer
	 */
	...
	protected function includes() {
		if ( ! class_exists( 'EDD_SL_Plugin_Updater' ) ) {
			require_once dirname( Alpha_Downloads()->path ) . '/classes/EDD_SL_Plugin_Updater.php';
		}
	}
	...
	public function add_settings_option( $options ) {
		$options[ $this->get_license_key() ] = array(
			'name'     =>  $this->name . ' ' . __( 'License Key', 'alpha-downloads' ),
			'tab'      => 'licenses',
			'type'     => 'text',
			'default'  => '',
	...
	protected function show_license_error() {
		...
		if ( isset( $status->error ) ) {
			switch ( $status->error ) {
				case 'missing':
					$message .= __( 'License key not found.', 'alpha-downloads' );
					break;
				case 'no_activations_left':
					$message .= __( 'No site activations remaining.', 'alpha-downloads' );
					break;
				case 'expired':
					$message .= __( 'Your license key has expired.', 'alpha-downloads' );
					break;
				default:
	...
	public function render_license_field() {
		...
		Alpha_Downloads()->render_view( 'license-field', array( 'key' => $key, 'value' => $value, 'active' => $active, 'status' => $status ) );
	}
